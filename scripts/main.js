	...
	var themeSettings= {
		...
		initArcticleSb2:function(){//Dynamic Sidebar Articles - tags 
			...
			$('#accordion .ry-link-item').each(function() {
				...
				// Sort <li> elements by <a> text in ascending (A–Z) order
				$lis.sort(function(a, b) {
					return $(a).find('a').text().trim().localeCompare($(b).find('a').text().trim());
				});

				// Re-append sorted <li>s into the <ul>
				$ul.append($lis);
			});
		},
	...
	*/
	...
	if(!$('.file-upload').length == 0){
		$(".file-upload").each(function() {
			...
			// Drag & Drop events
			$label[0].ondragover = function() {
				$label.addClass("changed");
				return false;
			};
			$label[0].ondragleave = function() {
				$label.removeClass("changed");
				return false;
			};
			$label[0].ondrop = function(e) {
				e.preventDefault();
				$label.removeClass("changed");
				var fileNames = e.dataTransfer.files;
				$label.find(".file_names").remove(); // clear previous
				for (var x = 0; x < fileNames.length; x++) {
					$label.append("<div class='file_names'>" + fileNames[x].name + "</div>");
				}
			};
			...
			$input.on("change", function() {
				...
				for (var i = 0; i < fileList.length; i++) {
					$label.append("<div class='file_names'>" + fileList[i].name + "</div>");
				}
				$label.addClass("changed");
			});
		});
	...
	var $items = $('.library-wrapper .ry-flex > .each');

	// Step 1: Remove any existing classes or data-types
	$items.removeClass('is-landscape is-portrait').removeAttr('data-type');

	// Step 2: Assign types dynamically based on row and column
	$items.each(function (index) {
		var $item = $(this);

		var rowIndex = Math.floor(index / 2); // 2 columns per row
		var colIndex = index % 2;

		// Alternating pattern:
		// Even row: col1 = portrait, col2 = landscape
		// Odd row: col1 = landscape, col2 = portrait
		var type = (rowIndex % 2 === 0)
		? (colIndex === 0 ? 'portrait' : 'landscape')
		: (colIndex === 0 ? 'landscape' : 'portrait');

		// Add data-type attribute and class
...
if(!$('div.ry-drop-nav.ry-link-item#services').length == 0){
	document.addEventListener('DOMContentLoaded', function() {
